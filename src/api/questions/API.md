...
- 上传时自动写入默认 metadata：
  ...
  - `tags = []`
  - `status = "none"`
  - `score`：从 TeX 文件中解析 `\begin{problem}[分数]{...}`，如果解析失败则为 `null`
  - `created_at = NOW()`
...
注意：`score`（题目总分）不能通过 PATCH 修改，只能通过 `PUT /questions/{id}/file` 替换 TeX 文件后自动更新。
...
- `score_min` / `score_max` 可单独使用，不需要 `difficulty_tag`
