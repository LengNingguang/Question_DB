...
pub(crate) async fn import_question_zip(
    ...
    query(
        ...
        INSERT INTO questions (
            question_id, source_tex_path, category, status, description, score, created_at, updated_at
        )
        VALUES (
            $1::uuid, $2, 'none', 'none', $3, $4, NOW(), NOW()
        )
        "#,
    )
    .bind(&question_id)
...
fn load_question_zip(zip_bytes: &[u8]) -> Result<LoadedQuestionZip> {
    ...
    let score = parse_score_from_tex(&tex_file.bytes);
...
/// Parse score from TeX content
/// Matches \begin{problem}[score]{title} or \problem[score]{title}
/// Returns None if no score is found (score is optional)
fn parse_score_from_tex(tex_bytes: &[u8]) -> Option<i32> {
    let tex_content = String::from_utf8_lossy(tex_bytes);
    let re = Regex::new(r"\\(?:begin\{problem\}|problem)\[(\d+)\]").ok()?;
    let score_str = re.captures(&tex_content)?.get(1)?.as_str();
    let score: i32 = score_str.parse().ok()?;
    // Validate score range (0-400)
